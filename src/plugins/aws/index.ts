  ...
  definePlugin({
    ...
    setup(cli: GetDotenvCli) {
      ...
      const cmd = cli
        ...
        .argument('[args...]')
        .action(async (opts: Record<string, unknown>, thisCommand: unknown) => {
          const self = thisCommand as { parent?: unknown; rawArgs?: string[] };
          ...
          // Strings/enums
          if (typeof opts.profile === 'string') overlay.profile = opts.profile;
