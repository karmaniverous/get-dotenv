...
You can define a scripts table in config and optionally override shell behavior
per script. Script strings are resolved by the `cmd` and `batch` commands:
...
When a script is invoked, `scripts[name].shell` (string or boolean) takes
precedence over the global `--shell` setting for that script.
